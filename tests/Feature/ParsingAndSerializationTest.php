...
test('it can parse large CSP headers and serialize to string again', function () {
    ...
    $result = Policyman::parse($hugeCSPHeader);
    $newHeader = Policyman::serialize($result);
    $newHeaderHash = md5($newHeader);
    expect($newHeaderHash)->toBe($headerHash);
});

test('it can validate a given header', function (string $header, bool $isValid) {
    expect(Policyman::validate($header))
        ->toBe($isValid);
})->with([
